...
def gram_matrix(ptime,L,Dt,kernel):
    '''
    ...
    K = np.empty((NL,len(ii),len(ptime)))
    for n,i in enumerate(reversed(range(1,NL+1))):
        tmp = ptime[ii] - i * Dt
        K[n] = kernel(tmp.reshape(-1,1),ptime.reshape(1,-1))
        K[n] = K[n] / np.sum(K[n],axis=1).reshape(-1,1)
    return K

def convert_to_FISTA(X,ptime,kernel,L,Dt,g):
    '''
    ...
    K = gram_matrix(ptime,L,Dt,kernel)
    NL = int(L/Dt)
    iiy = ptime > L
    Ny = np.sum(iiy)
    A = np.empty((Ny,NL * Ngenes)) 
    for i in range(NL):
        A[:,np.arange(Ngenes)*NL+i] = K[i] @ X.T
    y = X[g,iiy]
    return A,y,K
...
def single_gene_GLG(X,ptime,L,Dt,g,sigma,lambdas):
    k = lambda x,y: np.exp(-np.power(x-y,2)/sigma)
    A,y,K = convert_to_FISTA(X,ptime,k,L,Dt,g)
    nSamp, nFea = A.shape
    NL = int(L/Dt)
    ...
    fit = glmnet(x=A,y=y,family='gaussian',**opt)
    return fit

def GLG(X,ptime,L,Dt,lambdas,sigma):
    nGene, ntime = X.shape
    adj_matrix = np.zeros((len(lambdas),nGene,nGene))
    queue = np.arange(nGene)
    for g in range(nGene):
        fit = single_gene_GLG(X[queue,:],ptime,L,Dt,0,sigma,lambdas)
...
def main():
    ...
    adj = GLG(X,ptime,L,Dt,lambdas,sigma)
