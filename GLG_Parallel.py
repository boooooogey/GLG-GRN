...
def GLG_Parallel(inp):
    L, Dt, lambdas, sigma, probDrop, probZero, rep = inp
    ...
    for i in range(rep):
        tmp += GLG_network(X,ptime,L,Dt,lambdas,sigma,probDrop,probZero)
    lock.acquire()
    agg[:] = agg + tmp
    lock.release()
...
def parse_hyperparameters(path):
    ...
    list_hypes = []
    for i in hypes:
        lambdas = np.array([float(j) for j in i[1][8:len(i[1])-2].split(',')])
        Dt = float(i[2][3:])
        L = float(i[3][9:])
        sigma = float(i[4][13:])
        rep = int(i[6][10:])
        ...
        list_hypes.append((L*Dt, Dt, lambdas, sigma, probDrop, probZero, rep))
    return list_hypes
